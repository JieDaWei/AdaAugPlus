...
class Mixup_Token2:
    ...
    def __init__(self, mixup_alpha=1., cutmix_alpha=0., cutmix_minmax=None, prob=1.0, switch_prob=0.5,
                 mode='batch', correct_lam=True, label_smoothing=0.1, num_classes=2,
    ...
    def _params_per_batch(self):
        ...
        if self.mixup_enabled and np.random.rand() < self.mix_prob:
            if self.mixup_alpha > 0. and self.cutmix_alpha > 0.:
                ...
                assert False, "One of mixup_alpha > 0., cutmix_alpha > 0., cutmix_minmax not None should be true."
            lam = float(lam_mix)
        # print(lam,use_cutmix)
        return lam, use_cutmix

    def _mix_batch(self, x,y,label):
        ...
        if use_cutmix:
            ...
            # print('-------------')
            # print(mask.shape)
            # labelg = mask[0][0].detach().cpu().float().numpy()  
            # Image.fromarray(np.uint8(labelg*255)).save(os.path.join('/home/ubuntu/newdisk1/wjd/levir_TVT', "C", 'aaa.png'))
            mask_224 = torch.nn.functional.interpolate(mask, size=(256, 256), mode='nearest')

            x_flip = x.flip(0).mul_(mask_224)
            x.mul_(1 - mask_224).add_(x_flip)

            y_flip = y.flip(0).mul_(mask_224)
            y.mul_(1 - mask_224).add_(y_flip)

            label_flip = label.flip(0).mul_(mask_224)
            label.mul_(1 - mask_224).add_(label_flip)
            ...
            # label_flipped = label.flip(0).mul_(1. - lam)
            # label.mul_(lam).add_(label_flipped)
        return lam, mask

    @torch.no_grad()
    def __call__(self, x,y,label, target):
        assert len(x) % 2 == 0, 'Batch size should be even when using this'
        # img = x
        lam, mask = self._mix_batch(x,y,label)
        # lam, use_cutmix = self._params_per_batch()
        # print(lam)
        # x_flipped = x.flip(0).mul_(1. - lam)
        # x.mul_(lam).add_(x_flipped)
